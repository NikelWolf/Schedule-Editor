...
{
  logFile.reset(new QFile("/home/nick/Projects/log_test.txt")); //todo log file
  logFile.data()->open(QFile::Append | QFile::Text);
  qInstallMessageHandler(messageHandler);
  writeLog("Log init", log(), QtInfoMsg);
...
void Logger::writeLog(const QString message,
  ...
  switch (type)
    {
    case QtInfoMsg:
      qInfo(log) << message; break;
    case QtDebugMsg:
      qDebug(log) << message; break;
    case QtWarningMsg:
      qWarning(log) << message; break;
    case QtCriticalMsg:
      qCritical(log) << message; break;
    case QtFatalMsg:
